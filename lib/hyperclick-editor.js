...
class HyperclickEditor {
  constructor(editor, registry) {
    ...
    this.element = lumine.views.getView(editor);
    ...
    this.windowDisposables = new CompositeDisposable();
  ...
  listen(target, name, handler, capture = false, disposables = this.disposables) {
    target.addEventListener(name, handler, capture);
    disposables.add(new Disposable(() => target.removeEventListener(name, handler, capture)));
  }
  ...
  bindWindow() {
    ...
    this.listen(
      domWindow,
      "keydown",
      (event) => this.didChangeModifier(event),
      false,
      this.windowDisposables,
    );
    this.listen(
      domWindow,
      "keyup",
    ...
    this.listen(domWindow, "blur", () => this.clear(), false, this.windowDisposables);
  }

  beginWindowSurfaceTransition() {
    this.clear();
    this.windowDisposables.dispose();
    const finish = () => this.bindWindow();

  ...
  activate() {
    ...
    this.subscriptions.add(
      ...
      lumine.workspace.addWindowSurfaceTransitionObserver((context) => {
        const itemElement = lumine.views.getView(context.item);
        const controllers = [...this.editors.values()].filter(
          (controller) =>
            controller.element === itemElement || itemElement.contains(controller.element),
        );
        if (controllers.length === 0) return null;
        const participants = controllers.map((controller) =>
          controller.beginWindowSurfaceTransition(),
        );
        return {
          commit: () => {
          ...
          rollback: () => {
            for (const participant of participants) participant.rollback();
          },
        };
      }),
    );
